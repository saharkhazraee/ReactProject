...
import './styleWomanProductDetails.css';
import SimilarWomanProductSlider from './SimilarWomanProductSlider'



export default function WomanProductDetails() {
  ...
  useEffect(() => {
    (async () => {
      const res = await fetchData(`women/${id}?populate=*`);
      setProduct(res?.data);
    })();
  }, [id]);
  console.log(product)
  ...
  return (
    <Stack>
      ...
      <Box sx={{ width: '90%', margin: '10px auto'}}>
        ...
        <SimilarWomanProductSlider  catName={product?.attributes?.categories?.data[0]?.attributes?.name}/>
      </Box>
    </Stack>
  )
}
